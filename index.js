...
const mg = mailgun.client({
  username: "api",
  key: "50103e834f432cbe6875ccecee2db1cf-30b58138-f41cef7d",
});
...
async function run() {
  try {
    ...
    app.get("/sendmail", (req, res) => {
      mg.messages
        .create("sandbox74b7f364856a4d0299e9a78167a5d0ab.mailgun.org", {
          from: "Mailgun Sandbox <postmaster@sandbox74b7f364856a4d0299e9a78167a5d0ab.mailgun.org>",
          to: ["nabirasek@gmail.com"],
          subject: "Arif hacked",
          text: "Arif your account have been hackend",
        })
        .then((msg) => console.log(msg)) // logs response data
        .catch((err) => console.log(err));
      res.send("sendin mail");
    });
    ...
    app.get("/myProfile/:email", async (req, res) => {
      const email = req.params.email;
      const productsCount = await AllItem.countDocuments({
        Owner_email: email,
      });

      const upvotesCount = await Upvote.countDocuments({ Owner_email: email });

      const downvotesCount = await Downvote.countDocuments({
        Owner_email: email,
      });
    ...
    var WooCommerce = new WooCommerceAPI({
      url: 'https://swallowclothingbd.com/',
      consumerKey: 'ck_e5163f74396af809f7a7a4af6910473adf472469',
      consumerSecret: 'cs_3d105ea3d38d4d1be49246b2ddb7c8a6949ad578',
      wpAPI: true,
      version: 'wc/v3'
    });
